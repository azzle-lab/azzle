...
A service may charge callers for read APIs through x402, but it must label that payment as the service's fee, return V2 data from Base RPC, load token/address configuration from the canonical manifest or generated configuration, and never imply that an HTTP receipt changed onchain task state.

(function () {
  ...
  function formatWholeAzl(wei) {
    try {
      const whole = Number(BigInt(wei ?? "0") / 10n ** 18n);
      return Number.isFinite(whole) ? whole.toLocaleString() + " AZL" : " / ";
    } catch {
      return " / ";
    }
  }

  function setBoardValue(id, value) {
    const element = $(id);
    if (element) element.textContent = String(value);
  }

  async function loadMarketTotals() {
    try {
      const res = await fetch("/api/market/totals", { cache: "no-store" });
      const totals = await parseJsonResponse(res);
      if (!res.ok) throw new Error(totals.message || totals.error || "Market totals unavailable");
      const standard = totals.markets?.standard ?? {};
      const micro = totals.markets?.micro ?? {};
      setBoardValue("rd-market-board-completed", totals.completed ?? 0);
      setBoardValue("rd-market-board-open", totals.open ?? 0);
      setBoardValue("rd-market-board-claimed", totals.claimed ?? 0);
      setBoardValue("rd-market-board-active", totals.active ?? 0);
      setBoardValue("rd-market-board-disputed", totals.disputed ?? 0);
      setBoardValue("rd-market-board-resolved", totals.resolved ?? 0);
      setBoardValue("rd-market-board-locked", formatWholeAzl(totals.lockedAzlWei));
      setBoardValue("rd-market-board-total", totals.taskCount ?? 0);
      setBoardValue("rd-market-board-standard", standard.taskCount ?? 0);
      setBoardValue("rd-market-board-micro", micro.taskCount ?? 0);
      setBoardValue("rd-market-board-disputed-meta", totals.disputed ?? 0);
      setBoardValue("rd-market-board-resolved-meta", totals.resolved ?? 0);
      setBoardValue("rd-market-board-funded", formatWholeAzl(totals.fundedAzlWei));
      setBoardValue("rd-market-board-paid", formatWholeAzl(totals.releasedAzlWei));
      const completedNote = $("rd-market-board-completed-note");
      if (completedNote) completedNote.textContent = formatWholeAzl(totals.releasedAzlWei) + " paid to workers";
      const lockedUsd = $("rd-market-board-locked-usd");
      if (lockedUsd) lockedUsd.textContent = fmtUsd(totals.lockedAzlWei) + " ? Standard + Micro";
    } catch (error) {
      console.error("[market totals]", error);
    }
  }
  ...
  function bindRowClicks(tbody) {
    ...
    tbody.querySelectorAll(".rd-market-row").forEach((row) => {
      ...
      row.addEventListener("click", (event) => {
        if (event.target.closest("[data-claim-task]")) return;
  ...
  async function showClaimReadiness(api, taskId) {
    ...
    try {
      ...
      const accessFee = readiness.usesActionCredit
        ...
        ? "1 Action Credit will be used for this claim"
  ...
  function closeClaimConfirmation() {
    const modal = $("rd-claim-confirm-modal");
    if (!modal) return;
    modal.classList.remove("is-open");
    setTimeout(() => { modal.hidden = true; }, 180);
    pendingClaimTask = null;
  }

  async function openClaimConfirmation(task) {
    const modal = $("rd-claim-confirm-modal");
    const summary = $("rd-claim-confirm-summary");
    const fee = $("rd-claim-confirm-fee");
    const status = $("rd-claim-confirm-status");
    const submit = $("rd-claim-confirm-submit");
    if (!modal || !summary || !fee || !status || !submit) return;
    pendingClaimTask = task;
    modal.hidden = false;
    requestAnimationFrame(() => modal.classList.add("is-open"));
    summary.textContent = (taskMarket(task) === "micro" ? "Micro" : "Standard") +
      " task #" + displayTaskId(task) + " ? " + fmtAzl(taskAmount(task));
    fee.textContent = "Checking claim requirements...";
    status.textContent = "";
    submit.disabled = true;

    const api = walletApi();
    if (!api?.address) {
      fee.textContent = "Claim fee: " + (taskMarket(task) === "micro" ? "$0.50" : "$5") + " in oracle-priced AZL";
      status.textContent = "Sign in top-right before confirming.";
      return;
    }
    if (api.address.toLowerCase() === task.poster?.toLowerCase()) {
      status.textContent = "The task poster cannot claim their own task.";
      return;
    }
    try {
      const readiness = await api.claimReadiness(namespacedTaskId(task));
      fee.textContent = readiness.usesActionCredit
        ? "1 Action Credit will be used"
        : "Claim fee: " + (taskMarket(task) === "micro" ? "$0.50" : "$5") +
          " \u00b7 " + fmtAzl(Number(readiness.chargedAccessFeeAzl) * 1e18);
      const issues = [];
      if (!readiness.hasCollateral) issues.push("add " + fmtAzl(Number(readiness.shortfallAzl) * 1e18) + " collateral");
      if (!readiness.hasGas) issues.push("add ETH for Base gas");
      if (issues.length) {
        status.textContent = "Cannot claim yet: " + issues.join(" and ") + ".";
      } else {
        status.textContent = "Collateral and gas are ready. This transaction assigns the task to your wallet.";
        submit.disabled = false;
      }
    } catch (error) {
      status.textContent = error?.message ?? "Could not check claim requirements.";
    }
  }

  async function confirmPendingClaim() {
    if (!pendingClaimTask) return;
    const task = pendingClaimTask;
    const submit = $("rd-claim-confirm-submit");
    if (submit) submit.disabled = true;
    try {
      await claimTask(task, (message) => {
        const status = $("rd-claim-confirm-status");
  ...
  async function claimTask(task, onProgress) {
  ...
  function renderOpenRows(tasks) {
    ...
    tbody.innerHTML = tasks
      ...
      .map(
        ...
        (t) =>
          ...
          "<td><span class=\"rd-market-row-actions\"><button type=\"button\" class=\"rd-market-row-claim\" data-claim-task=\"" +
          escapeHtml(t.id) +
          "\">Claim <span class=\"rd-market-claim-dot\" aria-hidden=\"true\"></span> " +
          (taskMarket(t) === "micro" ? "$0.50" : "$5") +
          "</button><span class=\"rd-market-open-hint\">View</span></span></td>" +
    ...
    bindRowClicks(tbody);
    tbody.querySelectorAll("[data-claim-task]").forEach((button) => {
      button.addEventListener("click", async (event) => {
        event.preventDefault();
        event.stopPropagation();
        const task = v2Tasks.get(button.dataset.claimTask);
        if (!task) return;
        await openClaimConfirmation(task);
      });
    });
  ...
  function scheduleRefresh() {
    ...
    refreshTimer = setInterval(() => { loadMarketTotals(); loadCurrentView(); }, 120000);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("rd-market-refresh")?.addEventListener("click", () => { loadMarketTotals(); loadCurrentView(); });
    ...
    $("rd-claim-confirm-cancel")?.addEventListener("click", closeClaimConfirmation);
    $("rd-claim-confirm-backdrop")?.addEventListener("click", closeClaimConfirmation);
    $("rd-claim-confirm-submit")?.addEventListener("click", confirmPendingClaim);

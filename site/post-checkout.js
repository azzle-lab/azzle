(function () {
  ...
  function updatePostButton(status) {
    const button = $("rd-btn-post");
    if (!button) return;
    const note = button.querySelector(".rd-post-action-note");
    if (note) {
      note.textContent = status?.usesActionCredit
        ? "1 Action Credit will be used"
        : (status?.accessFeeAzl ? fmtAzlAmount(status.accessFeeAzl) : "Oracle-priced AZL access fee") +
          " ? " + money(eco().accessFeeUsd) + " per task";
    }
  }

  function renderActionCredit(status) {
    const panel = $("rd-action-credit-status");
    const title = $("rd-action-credit-title");
    const note = $("rd-action-credit-note");
    if (!panel || !title || !note) return;
    panel.hidden = false;
    const credits = Number(status?.wholeActionCredits ?? 0);
    if (status?.usesActionCredit) {
      panel.classList.add("is-ready");
      title.textContent = "1 Action Credit will be used";
      note.textContent = credits + " available on " + selectedMarket() + ". It will cover this post's access fee.";
    } else {
      panel.classList.remove("is-ready");
      title.textContent = "No Action Credit will be used";
      note.textContent = status?.stakingActive
        ? "This post will use the oracle-priced AZL access fee."
        : "Action Credits are not active on the selected " + selectedMarket() + " market.";
    }
  }
  ...
  async function refreshCheckout() {
    ...
    try {
      ...
      updatePostButton(status);
      renderActionCredit(status);

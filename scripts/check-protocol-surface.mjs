...
const X402_HANDLERS = [
  "azzle-open-tasks", "azzle-task", "azzle-task-scope", "azzle-reputation", "azzle-leaderboard",
  "azzle-union-overview", "azzle-deposit-usdc", "azzle-post-task", "azzle-claim-task",
  "azzle-stake", "azzle-unstake", "azzle-bank-credits", "azzle-claim-earnings",
];
for (const name of X402_HANDLERS) {
  const handler = await readFile(join(root, "agents", "x402-cloud", "x402", name, "index.ts"), "utf8");
  if (handler.includes("../manifest")) violations.push(`agents/x402-cloud/x402/${name}/index.ts: Bankr uploads index.ts only; inline the generated manifest`);
  if (!handler.includes("GENERATED by scripts/sync-manifest-surfaces.mjs") || !handler.includes(canonical.taskRegistry)) {
    violations.push(`agents/x402-cloud/x402/${name}/index.ts: missing inlined dual-market manifest`);
  }
  if (!handler.includes("new Response")) {
    violations.push(`agents/x402-cloud/x402/${name}/index.ts: Bankr runtime requires a Response, not a plain object`);
  }
}
const TASKS_SELECTOR = "0x8d977672";
const GET_TASK_SELECTOR = "0x1d65e77e";
for (const name of ["azzle-open-tasks", "azzle-task", "azzle-claim-task"]) {
  const handler = await readFile(join(root, "agents", "x402-cloud", "x402", name, "index.ts"), "utf8");
  if (handler.includes(GET_TASK_SELECTOR)) {
    violations.push(`agents/x402-cloud/x402/${name}/index.ts: use tasks(uint256) ${TASKS_SELECTOR}, not getTask`);
  }
  if (!handler.includes(TASKS_SELECTOR)) {
    violations.push(`agents/x402-cloud/x402/${name}/index.ts: missing tasks(uint256) selector`);
  }
}
...
if ((x402.currency ?? "USDC") !== "USDC" || x402.tokenAddress) {
  violations.push("agents/x402-cloud/bankr.x402.json: x402 Cloud must settle in USDC (no custom tokenAddress)");
}
for (const [name, service] of Object.entries(x402.services ?? {})) {
  if (service.tokenAddress) violations.push(`agents/x402-cloud/bankr.x402.json: ${name} must not set tokenAddress; prices are USDC`);
  if ((service.currency ?? x402.currency) !== "USDC") violations.push(`agents/x402-cloud/bankr.x402.json: ${name} must charge USDC`);
  const price = Number(service.price);
  if (!(price >= 0.01 && price <= 0.15)) violations.push(`agents/x402-cloud/bankr.x402.json: ${name} price must be 0.01–0.15 USDC`);
  if (!service.extensions?.bazaar?.info?.input) violations.push(`agents/x402-cloud/bankr.x402.json: ${name} must declare extensions.bazaar`);
  if (service.mimeType !== "application/json") violations.push(`agents/x402-cloud/bankr.x402.json: ${name} must declare application/json mimeType`);
}
for (const name of ["azzle-task-scope", "azzle-deposit-usdc", "azzle-post-task", "azzle-claim-task", "azzle-stake", "azzle-unstake", "azzle-bank-credits", "azzle-claim-earnings"]) {
  if (!x402.services?.[name]) violations.push(`agents/x402-cloud/bankr.x402.json: missing ${name}`);
}
const x402Services = JSON.parse(await readFile(join(root, "api", "lib", "x402-services.json"), "utf8"));
for (const name of Object.keys(x402.services ?? {})) {
  if (!x402Services[name]?.extensions?.bazaar?.info?.input) {
    violations.push(`api/lib/x402-services.json: ${name} missing bazaar discovery block`);
  }
}
for (const [name, service] of Object.entries(x402.services ?? {})) {
...
for (const name of ["azzle-task", "azzle-task-scope", "azzle-claim-task"]) {
...
for (const [name, selector] of [
  ["azzle-stake", "0xa694fc3a"],
  ["azzle-unstake", "0x8381e182"],
  ["azzle-bank-credits", "0x9a86de12"],
  ["azzle-claim-earnings", "0x1e83409a"],
]) {
  const handler = await readFile(join(root, "agents", "x402-cloud", "x402", name, "index.ts"), "utf8");
  if (!handler.includes(selector)) violations.push(`agents/x402-cloud/x402/${name}/index.ts: missing ${selector}`);
  if (!handler.includes("unsigned: true") || !handler.includes("transactions:")) {
    violations.push(`agents/x402-cloud/x402/${name}/index.ts: prepare endpoints must return unsigned transactions`);
  }
}

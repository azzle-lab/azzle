...
import { fetchAzlUsdPrice, azlTokensForUsd, azlWeiForUsd, azlCheckoutAllowed, formatAzlHuman } from "./azl-price.mjs";
import {
  loadPostingAccounts,
  savePostingAccounts,
  loadPostingQuotes,
  savePostingQuotes,
} from "./posting-store.mjs";
import { PLANS, AZL_PAY_DISCOUNT, QUOTE_TTL_MS } from "./posting-plans.mjs";
...
export async function recordPost(address, { taskId, txHash, description, budgetUsdc, deadlineDays, discoveryOpen, market = "standard" } = {}) {
  ...
  const store = await loadStore();
  const storeKey = accountKey(addr, selected);
  const user = store.users[storeKey] ?? { tier: "free", dailyPosts: {} };

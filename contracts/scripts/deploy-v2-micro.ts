...
const COMPONENT_NAMES = [
  "pricingPolicy", "depositVault", "escrowVault", "reputationRegistry", "verifierBondVault",
  "stakingVault", "treasuryRouter", "taskRegistry", "arbitrationModule", "usdcWethLeg",
  "exactInputExecutor", "paymentGateway", "taskScopeRegistry",
] as const;

const GOVERNANCE_SAFE = ethers.getAddress("0xB459145b74Ca4B198f73C0d573a161e85CA76D27");
const RELEASE_NAMESPACE = process.env.V2_RELEASE_NAMESPACE?.trim() || "AZZLE_V2_MICRO";
...
const ARTIFACT_BASENAME = process.env.V2_ARTIFACT_BASENAME?.trim() || "base-8453-micro";
...
const DEFAULTS = {
  ...
  minimumVerifierBondAzl: ethers.parseEther("1000"),
  openTaskCapUsd6: 2_500_000_000n,
  maxTaskUsd6: 50_000_000n,
  entryDepositUsd6: 3_000_000n,
  liveTaskReserveUsd6: 1_000_000n,
  accessFeeUsd6: 500_000n,
  exitCompensationUsd6: 250_000n,
  exitProtocolShareUsd6: 250_000n,
  maxUsdcInput6: 100_000_000n,
  maxEthInput: ethers.parseEther("0.05"),
  creditCap: ethers.parseEther("6000000"),
  creditBaseStake: ethers.parseEther("10000000"),
  creditContext: ethers.id("AZZLE_V2_MICRO_AGENT_DEPOSIT"),
} as const;
...
async function main() {
  ...
  const factoryContractFactory = await ethers.getContractFactory("AzzleSuiteV2MicroFactory");
  ...
  if (phase === "stage") {
    ...
    factory = await ethers.getContractAt("AzzleSuiteV2MicroFactory", factoryAddress);
  ...
  predicted.treasuryRouter = ethers.getCreate2Address(
    factoryAddress, salts[6], ethers.keccak256(treasuryInitCode)
  );

  predicted.observationOracle = ethers.getAddress(canonicalManifest.observationOracle);
  predicted.twapAdapter = ethers.getAddress(canonicalManifest.twapAdapter);
  predicted.usdOracle = ethers.getAddress(canonicalManifest.usdOracle);
  ...
  const actionCredits = {
    ...
    lifetimeCap: "6000000000000000000000000", baseStakeAzl: "10000000000000000000000000",
  ...
  if (phase === "stage") {
    if (!factory) {
      const deployedFactory = await factoryContractFactory.deploy(
        canonicalManifest.observationOracle,
        canonicalManifest.twapAdapter,
        canonicalManifest.usdOracle,
      );
  ...
  if (phase === "handoff-artifact" || phase === "launch-artifact" || phase === "promote") {
    ...
    if (phase === "promote") {
      ...
      const canonical = path.resolve(__dirname, "../deployments/base-8453-micro.json");
      const liveStandard = path.resolve(__dirname, "../deployments/base-8453.json");
      if (path.resolve(canonical) === path.resolve(liveStandard)) {
        throw new Error("micro promotion must not write the live standard manifest");
      }
      writeExclusiveAtomic(canonical, candidate);
      console.log(`Micro candidate promoted to ${canonical}`);
      return;
    }
    const suite = await factory.deployedSuite();
    const owned = [
      suite.depositVault, suite.escrowVault, suite.reputationRegistry,
    ...
    if (phase === "handoff-artifact") {
      writeExclusiveAtomic(V2_HANDOFF_ARTIFACT, {
        ...
        meta: { name: "AZZLE micro ownership acceptance", txBuilderVersion: "1.18.0", createdFromSafeAddress: governance },
      ...
      console.log(`Micro ownership Safe artifact written to ${V2_HANDOFF_ARTIFACT}`);
      return;
    }
    const gateway = await ethers.getContractAt("AzlPaymentGateway", suite.paymentGateway);
    const staking = await ethers.getContractAt("UnionStakingVaultV2", suite.stakingVault);
    const baseArtifact = {
      version: "1.0", chainId: BASE.chainId.toString(),
      meta: { name: "AZZLE micro market activation", txBuilderVersion: "1.18.0", createdFromSafeAddress: governance },
    };
    writeExclusiveAtomic(V2_LAUNCH_ARTIFACT.replace(".json", "-unpause.json"), {
      ...baseArtifact,
      meta: { ...baseArtifact.meta, name: "AZZLE micro intake activation" },
      prerequisites: [
        "Accept all micro-graph ownership transfers.",
        "Shared standard TWAP/USD oracle is already live.",
        "Graph/manifest checks pass.",
      ],
    ...
    writeExclusiveAtomic(V2_LAUNCH_ARTIFACT.replace(".json", "-activate-staking.json"), {
      ...baseArtifact,
      meta: { ...baseArtifact.meta, name: "AZZLE micro Union staking activation" },
      prerequisites: ["Ownership is accepted.", "Intake may be paused or open independently."],
      transactions: [{ to: await staking.getAddress(), value: "0", data: null, contractMethod: { inputs: [], name: "activateStaking", payable: false }, contractInputsValues: {} }],
    });
    console.log(`Micro launch Safe artifacts written beside ${V2_LAUNCH_ARTIFACT}`);
    return;
  }
  ...
  if (phase === "bond-check") {
    if ((await factory.deploymentPhase()) < 1n) throw new Error("VerifierBondVaultV2 is not deployed yet");
  ...
  if (phase === "verify-deployed") {
    ...
    const definitions: Array<{ name: string; contract: string; args: unknown[] }> = [
      {
        name: "AzzleSuiteV2MicroFactory",
        contract: "src/v2/AzzleSuiteV2MicroFactory.sol:AzzleSuiteV2MicroFactory",
        args: [canonicalManifest.observationOracle, canonicalManifest.twapAdapter, canonicalManifest.usdOracle],
      },
    ...
    const deployedCount = phaseNow === 0 ? 0 : phaseNow === 1 ? 5 : phaseNow === 2 ? 9 : 13;
  ...
  if (phase === "finalize") {
    ...
    const manifest = {
      version: "2.0.0", chainId: BASE.chainId.toString(), market: "micro",
